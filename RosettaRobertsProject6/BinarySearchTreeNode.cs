...
namespace RosettaRobertsProject6
{
	...
	public static class BinarySearchTreeNode
	{
		...
		{
			return root.GetStringOfAllValuesInInfixOrder();
		}
		...
		public static string ToDisplayString<T>(this BinarySearchTreeNode<T> root)
		{
			...
			string Row(BinarySearchTreeNode<T> node, int depth, string accum)
			{
				...
				if (depth == 0)
					return Row(node.Right, depth - 1, Row(node.Left, depth - 1, accum) + node.Value + " ");
		...
		/// <summary>
		///     Returns a lisp string of this tree. Returns () for an empty tree.
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <param name="root"></param>
		/// <returns></returns>
		public static string ToLispString<T>(this BinarySearchTreeNode<T> root)
		{
			if (root == null) return "()";
			...
			return "("
			       ...
			       {
				       root.Value, root.Right?.ToLispString(), root.Left?.ToLispString()
			       }.Where(s => s != null))
			       + ")";
		}
		...
		/// <param name="root"></param>
		/// <returns></returns>
		public static int MaxDepth<T>(this BinarySearchTreeNode<T> root)
		{
		...
		/// <param name="root"></param>
		/// <returns></returns>
		public static int MinDepth<T>(this BinarySearchTreeNode<T> root)
		{
			return MinDepth(root, 0, int.MaxValue);
		}
